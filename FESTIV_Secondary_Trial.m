...
clear all;
load('PES_21');
%% Overwriting the number of days for simulation
daystosimulate=2;
%NETWORK_CHECK='NO';
%inputPath='/home/jxr180022/FESTIV_MODEL-master/Input/NREL_118_BUS_System/NREL_118_BUS_IES_APRIL.xlsx';% put your file name with detailed path address
inputPath='/home/jxr180022/FESTIV_MODEL-master_LTE/Input/NREL_118_BUS_NEW/NREL_118_BUS_IES_H2_7_DAYS_LTE.xlsx';
%inputPath='/home/jxr180022/FESTIV_MODEL-master/Input/NREL_118_BUS_NEW/NREL_118_BUS_N_IES_Site.xlsx';
a=inputPath;
name='NREL_118_BUS_IES_H2_7_DAYS_LTE.xlsx';% update the file name only
%name='NREL_118_BUS_IES.xlsx';
DH_FLAG=1;
Thermal_Load_Flag=0;
displayname=name;
%%
...
%% Data and Initialization
    ...
    %INPUT_FILE_VALIDATION % Just commenting it out to see whether it works or not
    ...
    % This is for cutting down the runtime of Texas-2000 bus system where
    % actually the 3-dimensional branchbus matrix is reduced to
    % 2-dimensional PTDF matrix
    ...
    % DH
    if DH_FLAG==1
        DH_LOAD_VAL=GATHER_LOAD_INPUT_FOR_SCHEDULING_PROCESS(DAC_DH_LOAD_FULL,DASCUC_binding_interval_index,HDAC);
    end
    ...
    % DH
    if DH_FLAG==1
        DH_LOAD_VAL=GATHER_LOAD_INPUT_FOR_SCHEDULING_PROCESS(RTC_DH_LOAD_FULL,RTSCUC_binding_interval_index,HRTC);
    end
    %Gather VG Forecasts
   ...
   % DH
    if DH_FLAG==1
        DH_LOAD_VAL=GATHER_LOAD_INPUT_FOR_SCHEDULING_PROCESS(RTD_DH_LOAD_FULL,RTSCED_binding_interval_index,HRTD);
    end
...
%% Real-Time SCED
    if rtsced_update + eps >= tRTD  
        ...
        if Stop_for_Infeasibilities && numberOfInfes ~= 0 && max(rpu_time) < time - PRTD/60 && max(rpu_time) < time
            LTwarning('stophere:RTDinfeasible', 'Infeasible RTD Solution');
        end
        catch
        end;
